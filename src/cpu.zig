...
pub const Z80 = struct {
    ...
    // Trap handler
    trap: ?*const fn (*Z80, address: u16) u8,
    ...
    pub fn getTrapOrOpcode(self: *Z80) u8 {
        if (self.peekByte() == 0x64 and self.trap != null) {
            const opcode = self.trap.?(self, self.pc - 1);
            if (opcode != 0x00) {
